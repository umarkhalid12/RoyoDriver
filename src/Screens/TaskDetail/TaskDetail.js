...
useEffect(()=>{
  updateState({
    pickupattribute: !!taskDetail?.pickup_attribute? JSON.parse(taskDetail?.pickup_attribute):[],
    dropattribute: !!taskDetail?.dropoff_attribute? JSON.parse(taskDetail?.dropoff_attribute):[],
  })
},[])
  ...
  useEffect(()=>{
    console.log('tetststts',taskStatus)
    if(taskStatus==3){
      setIsRunning(TurboModuleRegistry)
    }
  },[taskStatus])
useEffect(()=>{
  if(time>=300){
...
console.log(taskStatus,'taskStatustaskStatus', taskDetail?.tasktype);
// && taskDetail?.tasktype?.id == 1
  useEffect(()=>{
      if (taskStatus == 2 ) {
        Platform?.OS == 'android' ? openGoogleMap() : openMaps()
      }
  }, [taskStatus])
    ...
    actions
      ...
      .then(res => {
        ...
        if (res?.data) {
          ...
          console.log(taskStatus,'statususuusus')
          // getStatusName(taskStatus)
          if(taskStatus == 2){
            startStopwatch()
          }
  ...
  const buttonView = () => {
    ...
    return (cancelRequestExit || paramData?.is_from_calendar) ? null : (
      ...
      <View style={styles.container}>
        <TouchableWithoutFeedback
          onPressIn={taskStatus == 2 ? redirectToDoneScreen : handlePressIn}
        ...
        <View style={{}}>
          ...
          {(taskDetail?.tasktype?.name).toLowerCase() == 'drop' ? (
            ...
            <View
              ...
              <View style={{marginHorizontal:moderateScale(14),marginVertical:moderateScaleVertical(8)}}>
               {!isEmpty(pickupattribute)&& <View>
              <Text style={{fontFamily:fontFamily.bold,fontSize:textScale(14)}}>Pickup Instruction</Text>
              {pickupattribute.map((val,index)=>{
                                        return(
                                      <View style={{flexDirection:'row',alignItems:'center'}}>
                                        <Text
                                        style={{
                                     ...
                                     {!isEmpty(dropattribute)&&  <View >
                                       <Text style={{fontFamily:fontFamily.bold,fontSize:textScale(14)}}>Drop Instruction</Text>
              {dropattribute.map((val,index)=>{
                                        return(
                                      <View style={{flexDirection:'row',alignItems:'center'}}>
                                        <Text
                                        style={{
            ...
            />} */}
             {(taskStatus == 3) &&
              <View style={{
                ...
                <Text style={{fontSize:textScale(12),marginTop:moderateScaleVertical(6)}}>Please do not go back from this Screen!</Text>
              </View>}
            {false && (
              <ButtonComponent
                buttonStyle={{
        ...
        <View
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            {!!Number(taskDetail?.order?.cash_to_be_collected) > 0 && (
              <View style={{ flex: 0.4 }}>
                <Text style={styles.taskLable}>
                  {strings.DRIVER_COMMISSION.toUpperCase()}
                </Text>
                <Text
                  numberOfLines={1}
                  style={[styles.emailAndPhone, { marginTop: moderateScale(5) }]}>
                  {Number(taskDetail?.order?.driver_cost).toFixed(2)}
                </Text>
              </View>
            )}
          </View>
    ...
    <WrapperContainer
      ...
      <Header
        ...
        customRight={() =>
          ...
          )||((taskStatus == 3 || taskDetail?.tasktype?.name == "Drop") && !isRunning ) && (
